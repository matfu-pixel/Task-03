...
menu = [{'name': 'Main', 'url': '/index'},
        ...
        {'name': 'Add data', 'url': '/load_dataset/False'},
...
@app.route('/load_dataset/<error>', methods=['GET', 'POST'])
def add_data(error=False):
    """
    page for load data
    """
    try:
        ...
        if file_form.validate_on_submit(): # check csrf_token
            ...
            return redirect(url_for('add_data', error=error))

        return render_template('add_data.html', form=file_form, menu=menu, page_title='Load dataset', error=error)
    except Exception as exc:
        app.logger.info('Exception: {0}'.format(exc))
        return redirect(url_for('add_data', error=True))


@app.route('/add_model', methods=['GET', 'POST'])
def add_model():
    """
    select model for fitting page
    """
    try:
...
def choose_model_data():
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            return redirect(url_for('show_info', data_name=data_name, model_name=model_name, error=False))

        return render_template('choose_model_data.html', form=form, menu=menu, page_title="Choose model and data")
    except Exception as exc:
        app.logger.info('Exception: {0}'.format(exc))
        return redirect(url_for('choose_model_data'))


@app.route('/show_info/<data_name>/<model_name>/<error>', methods=['GET', 'POST'])
def show_info(data_name=None, model_name=None, error=False):
    """
    ...
    try:
        ...
        return render_template('show_info.html', form=form, menu=menu, page_title="Information about model", data_name=data_name, model_name=model_name, model=models[model_name], error=error)
    except Exception as exc:
        app.logger.info('Exception: {0}'.format(exc))
...
def plot_predict(data_name=None, model_name=None):
    """
    plot graphic RMSE-n_estimators and make prediction of test.csv
    """
    try:
...
def about_me():
    ...
    """
    return render_template('about_me.html', menu=menu, page_title='About me')
